...
const handler = async (m, {conn, text, participants, args}) => {
  ...
  if (!global.db.data.settings[conn.user.jid].restrict) throw translator.text1;
  if (!args[0]) throw translator.text2;
  ...
  try {
    ...
    for (const user of participant.filter((item) => item.attrs.error == 403)) {
      ...
      const textMsg = `${translator.text3[0]} @${jid.split('@')[0]}, ${translator.text3[1]}`;
      
      m.reply(textMsg, null, {mentions: conn.parseMention(textMsg)});
      
      const caption = translator.text4;
      const message = await prepareWAMessageMedia({image: jpegThumbnail}, {upload: conn.waUploadToServer});
