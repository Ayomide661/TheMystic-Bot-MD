const handler = async (m, { conn }) => {
  ...
  if (testi && testi.length > 0) {
    ...
    if (global.db.data.chats[m.chat].isBanned) {
      m.reply('⚠️ This chat is already banned.');
      return;
    }

    global.db.data.chats[m.chat].isBanned = true;
    m.reply(`✅ Bot ${conn.user.name || 'current'} has been specifically banned from this chat.`);
    return;
  }
  ...
  global.db.data.chats[m.chat].isBanned = true;
  m.reply(translator.texto1 || '✅ Chat banned successfully.');
};
