...
const handler = async (m, {conn, args, usedPrefix, __dirname}) => {
  ...
  if (user.health >= 100) return conn.reply(m.chat, `${tradutor.text1} ${user.health}`, m);
...
[`🏕️ 𝙰𝚅𝙴𝙽𝚃𝚄𝚁𝙰𝚁`, `${usedPrefix}adventure`]], m)*/
  ...
  if (user.potion < count) {
    return conn.reply(m.chat, `*${htki} ${tradutor.text2[0]} ${htka}${tradutor.text2[1]}  ${count - user.potion} ${tradutor.text2[2]} 
${tradutor.text2[3]}  ${user.health} ❤️
${tradutor.text2[4]}  ${user.potion} 🥤
${tradutor.text2[5]} `, m);
  }
...
*${usedPrefix}transfer potion ${count - user.potion}* @${conn.getName(m.sender)}`]], m)*/
  ...
  conn.reply(m.chat, `${tradutor.text3[0]} ${count} ${tradutor.text3[1]} ${user.health} ${tradutor.text3[2]}`, m);
};
