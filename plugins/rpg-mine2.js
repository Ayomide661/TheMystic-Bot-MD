...
const handler = async (m, {conn, isPrems}) => { // lastmiming
  ...
  if (new Date - user.lastcoins < 600000) return await conn.reply(m.chat, `${tradutor.text2[0]} ${msToTime(time - new Date())} ${tradutor.text2[1]}  ${global.rpgshopp.emoticon('money')}⛏️*`, m);
  user.money += premium ? moneypremium : money;
  let text = '';
  for (const reward of Object.keys(recompensas)) {
    if (!(reward in user)) continue;
    user[reward] += recompensas[reward];
    text += `+${recompensas[reward]} ${global.rpgshop.emoticon(reward)}\n`;
  }
  const text = `*${premium ? tradutor.text3[0] : tradutor.text3[1]}*\n*${minar}*\n*${money} ${global.rpgshop.emoticon('money')}*\n\n${tradutor.text3[2]}n${text}\n\n${tradutor.text3[3]} ${premium ? '✅' : '❌'}\n${wm}`;
